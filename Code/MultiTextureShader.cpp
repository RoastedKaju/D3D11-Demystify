...
bool MultiTextureShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX projection, ID3D11ShaderResourceView* textureA, ID3D11ShaderResourceView* textureB)
{
	bool result;

	result = SetShaderParameters(deviceContext, world, view, projection, textureA, textureB);
	if (!result)
	{
		return false;
	}
...
bool MultiTextureShader::InitializeShader(ID3D11Device* device, HWND hwnd, const char* vsFilename, const char* psFilename)
{
	HRESULT result;
...
void MultiTextureShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const char* shaderFilename)
{
	char* compileErrors;
	size_t bufferSize;
	std::ofstream fout;
...
bool MultiTextureShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* textureA, ID3D11ShaderResourceView* textureB)
{
	HRESULT result;
	...
	// slot 0 gets textureArray[0], slot 1 gets textureArray[1].

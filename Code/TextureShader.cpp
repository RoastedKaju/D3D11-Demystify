...
{
	return InitializeShader(device, hwnd, "Shaders/Texture.vs", "Shaders/Texture.ps");
}
...
bool TextureShader::InitializeShader(ID3D11Device* device, HWND hwnd, const char* vsFilename, const char* psFilename)
{
	HRESULT result;
	...
	result = D3DCompileFromFile(Utils::ConvertToWChar(vsFilename), NULL, NULL, "TextureVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	result = D3DCompileFromFile(Utils::ConvertToWChar(psFilename), NULL, NULL, "TexturePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	polygonLayout[1].Format = DXGI_FORMAT_R32G32_FLOAT;      // just U,V - two floats
	...
	// The sampler state - how Sample() in the pixel shader behaves. This is
	samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;     // smooth bilinear/trilinear filtering
	samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;        // tile the texture past 0..1 UVs
...
bool TextureShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	// Bind the shader resource view to the pixel shader stage, at the slot
	// matching "Texture2D shaderTexture" (register t0) in Texture.ps.
...
void TextureShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	// Bind the sampler state, matching "SampleType" (register s0) in Texture.ps.
